Ponder.registry((event) => {
  event
    .create(["minecraft:mushroom_stem","minecraft:red_mushroom_block"])
    .tag("kubejs:block_filling_basic")
    .scene(
      "kubejs:red_mushroom_block",
      "使用氮肥注液转化红色蘑菇",
      "kubejs:9x9base",
      (scene, utils) => {
        ...
        let Inputs = ["minecraft:red_mushroom"]
        let Outputs = ["minecraft:mushroom_stem","minecraft:red_mushroom_block"]
        ...
        scene.idle(20);
        scene.text(120, "方块种类要求：红色蘑菇", [6.5, 1, 1.5]);
        scene.overlay.showOutline("red", { "glue": true }, [6, 1, 1, 8, 1, 7], 120);
        scene.text(120, "方块产出：红色蘑菇方块、蘑菇柄", [3.5, 1.5, 8.5]);
